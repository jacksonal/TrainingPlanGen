"""Unit test case for DanielsRunningTrainingPlanGenerator"""
...
class TestDanielsRunningFormula(unittest.TestCase):
    ...
    def setUp(self):
        """Setup for testing."""
        self.generator = DanielsRunningTrainingPlanGenerator()
    ...
    def test_r_pace(self):
        ...
        #2:16 R pace
        pace = DanielsRunningTrainingPlan.get_R_pace(64)
        self.assertAlmostEqual(71, pace, delta=2)

        #1:42 R Pace
        pace = DanielsRunningTrainingPlan.get_R_pace(42)
        self.assertAlmostEqual(102, pace, delta=2)

        #62 R Pace
        pace = DanielsRunningTrainingPlan.get_R_pace(74)
        self.assertAlmostEqual(62, pace, delta=2)

        #2:02 R Pace
        pace = DanielsRunningTrainingPlan.get_R_pace(34)
        self.assertAlmostEqual(122, pace, delta=4)

        #80 R Pace
        pace = DanielsRunningTrainingPlan.get_R_pace(56)
        self.assertAlmostEqual(80, pace, delta=2)

    def test_print_workout(self):
        w = DanielsRunningTrainingWorkout()
        w.desc = "E"
        self.assertEqual(w.desc, '%s' % w)
        pass

    def test_print_day(self):
        d = DanielsRunningTrainingDay()
        d.set_day_of_week(1)
        w = DanielsRunningTrainingWorkout()
        w.desc = 'E'
        d.add_workout(w)
